...
# Section 3: Identity & Federation - IAM Access Analyzer
...
> You can also share CIDR prefix rules. This allows you to find a prefix of IPs 10.1.255.\*, which can be referenced in Security Groups. So child account 1 can not see SG in other accounts but we can build simpler rules by using IP prefix. **Updates will propagate** so if we need to make changes to the prefix lists, the SG will automatically be updated.
